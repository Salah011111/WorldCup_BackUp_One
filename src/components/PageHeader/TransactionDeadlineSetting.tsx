  ...
  return (
    <Box mb="16px">
      <Flex alignItems="center" mb="8px">
        <Text bold>{translateString(90, `${t('Transaction deadline')}`)}</Text>
        <QuestionHelper
          text={translateString(188, `${t('Your transaction will revert if it is pending for more than this long.')}`)}
        />
      </Flex>
      <Field>
        <Input type="number" step="1" min="1" value={value} onChange={handleChange} />
        <Text fontSize="14px" ml="8px">
        <Text bold>{translateString(90, `${t('Minutes')}`)}</Text>
        </Text>
      </Field>
      {error && (
