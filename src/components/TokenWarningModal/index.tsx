          ...
          {chainId && (
            <ExternalLink style={{ fontWeight: 400 }} href={getBscScanLink(chainId, token.address, 'token')}>
              <Text title={token.address}>
                {shortenAddress(token.address)} {TranslateString(116, `${t('View on BscScan')}`)}
              </Text>
            </ExternalLink>
          )}
        </AutoColumn>
    ...
    <Modal isOpen={isOpen} onDismiss={handleDismiss} maxHeight={90}>
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          <AutoRow gap="6px">
            <StyledWarningIcon />
            <Text color="failure">{TranslateString(1128, `${t('Token imported')}`)}</Text>
          </AutoRow>
          <Text>
            {TranslateString(
              1130,
          ...
          <Text>{TranslateString(1134, `${t('If you purchase an arbitrary token, you may be unable to sell it back.')}`)}</Text>
          {tokens.map((token) => {
          ...
          <RowBetween>
            <div>
              <label htmlFor="understand-checkbox" style={{ cursor: 'pointer', userSelect: 'none' }}>
                ...
                />{' '}
                <Text as="span" ml="4px">
                  {TranslateString(148, `${t('I understand')}`)}
                </Text>
              </label>
            </div>
            <Button
            ...
            >
              {TranslateString(150, `${t('Continue')}`)}
            </Button>
          </RowBetween>
        </AutoColumn>
      </WarningContainer>
